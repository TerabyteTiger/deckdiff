<template>
  ...
  <main class="grid grid-cols-1 xl:grid-cols-2 mb-24">
    ...
    <button
      @click="compare()"
      class="px-4 py-2 bg-blue-300 border-2 border-blue-800 mx-auto rounded my-8 xl:col-span-2"
    >
      Compare
    </button>
    <div class="px-8">
      <h2 class="text-2xl font-bold underline">Remove:</h2>
      <ul class="list-disc list-inside text-red-800 font-bold text-lg pl-4">
        <li v-for="(card, index) in Object.keys(removeList)" :key="index">
          {{ removeList[card] }} {{ card }}
        </li>
      </ul>
    </div>
    <div class="px-8">
      <h2 class="text-2xl font-bold underline">Add:</h2>
      <ul class="list-disc list-inside text-green-800 font-bold text-lg pl-4">
        <li v-for="(card, index) in Object.keys(addList)" :key="index">
          {{ addList[card] }} {{ card }}
        </li>
      </ul>
    </div>
